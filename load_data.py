...
def load_data():
    extensions = ["jpg", "png"]
    males = "images/males/male"
    females = "images/females/female"
    others = "images/others/other"
    images = []
    for i in range(1, 51):
        for extension in extensions:
            path = males + ' (' + str(i) + ').' + extension
            try:
                image = Image.open(path)
                images.append([image, 1])
            except FileNotFoundError:
                print("not found", path)
            path = females + ' (' + str(i) + ').' + extension
            try:
                image = Image.open(path)
                images.append([image, 1])
            except FileNotFoundError:
                print("not found", path)
            path = others + ' (' + str(i) + ').' + extension
            try:
                image = Image.open(path)
...
def get_data_set(train_test_ratio=0.8):
    ...
    # print(image_list, image_classes)
    # data_set = ImageClassifierDataset(image_list, image_classes)
    # print(data_set[29])
